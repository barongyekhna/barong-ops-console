...
def marketing_copy_instruction(channel: str) -> str:
    ...
    common = (
        ...
        "copy. COVERAGE RULE: every phrase in `final_keywords` must appear as an exact "
        "phrase at least once somewhere in customer-facing copy (an H2 heading or a "
        "body sentence) — the highest-volume keywords are the reason this page exists; "
        "weave them naturally, never keyword-stuff or force an "
    ...
    return common + (
        ...
        "FAQ RULE (absolute): when `faq_question_clusters` is non-empty, choose 2-3 "
        "distinct clusters, copy each selected cluster's preferred_question exactly, "
        "and cite only its preferred_evidence_refs source IDs. Never invent or paraphrase "
        "a question or provenance ID. Return an empty page_faq when that cluster list is "
        "empty. Never restate a basic specification as a question. "
        "Serper evidence only proves the question is REAL (worth answering); it does NOT "
        "supply the answer. Ground EVERY answer solely in THIS product's own verified facts "
        "— approved selling points, structured_specs_json, package_includes, and how this "
        "product actually works. Do NOT answer with generic category behaviour from the "
        "search snippets: when a snippet describes how a typical device works but this "
        "product works differently, answer according to THIS product's real mechanism, never "
        "the generic one (e.g. if the specs show a submersible bottom-suction pump that sits "
        "in the water, never describe a separate intake hose or priming step it does not "
        "have). If a sourced question cannot be answered truthfully from this product's own "
        "facts, or its honest answer would contradict the specs/mechanism, DROP that cluster "
        "rather than write a generic or speculative answer. Answers give method, reassurance, "
        "or tradeoffs and MUST NOT repeat any product-specific numeric value from "
        "structured_specs_json (including dimensions, weight, capacity, or piece count); use "
        "qualitative wording such as 'nests compactly' and leave exact numbers to the "
...
def image_art_direction_instruction() -> str:
    return (
        ...
        "Add at least three separate placement=description proof_scene images for the "
        "horizontal description modules; they do not satisfy either gallery proof_scene slot. "
