...
**独立站（Barong Yekhna · 7-8 张，顺序即漏斗 —— 是"信息漏斗"，不是"一堆好看图"）：**

顶级 DTC 的产品图每张都在打消一个具体疑虑、传一条具体信息。**每张图都必须携带产品信息**（苹果、任何像样电商都不会让全部图零信息）。

| # | 图位（asset_role） | 使命 | 要点 |
|---|---|---|---|
| 1 | 主图 `main`（白底） | 点击 | 家规干净产品图，产品是唯一变量 |
| 2 | 卖点信息图 `feature_callout` | 看懂 | 干净产品基底 + **程序化叠加** 3-5 条真实卖点标注（引线指向对应部位）；文字来自**真实规格**（色温 2700K / IP 等级 / 续航 h / mAh），≤30 词 |
| 3 | 尺寸图 `dimension` | 打消疑虑 | 干净产品基底 + **程序化叠加尺寸线与数值**（长宽高）+ 真实参照物（手/鞋/石板旁）；最强降退货图 |
| 4 | 配件/开箱图 `accessory` | 打消疑虑 + 分层打击 | 平铺"包含什么"（主体 + 各配件，如**尖地插 vs 平底座**），让不同使用面（草地/石板）的顾客都放心 |
| 5 | 材质特写 `detail` | 信任 + 差异化 | 微距特写核心差异点（手吹玻璃纹理/太阳能板/接口），放大工艺与"独一无二"的情绪价值 |
| 6-7 | 场景图 `scene`（1-2 张，精） | 想要 | 真实环境使用中的"向往时刻"，家规暖亮柔光；**宁缺毋滥**，别一堆低质场景 |

**图上文字规则（硬约束）：** `feature_callout` / `dimension` / `spec` 类图 = AI 只出**干净基底图 + 留出标注空间**；**文字、引线、尺寸线一律由管线程序化叠加**（AI 直接写字必乱码——见下方红线）。因此这几类图的作图指令必须额外输出一段结构化 `overlay`：
- 严格使用 `k-info-overlay-v1`：顶层包含 `schema_version`、`role`、`items`；每条 item 只给 `type`、`source_field` 与相对坐标/引线方向。`source_field` 引用规格字段（如 `color_temperature_k` 或 `dimensions.height`）。
- **不得输出 `text`、`label` 或数值字段**；显示标签由服务端固定映射，显示数值由 K worker 从规格证据解析。
- 数据来源 = 产品**真实规格**（1688 抓取，落在 K 产品 `structured_specs_json`）；抓不到的字段**不叠、不编**。
- 消费方 = K 图片渲染 worker 的确定性叠字合成步骤；失败时保留干净基底图并记录结果，不阻断后续上架。

**场景图反"假图层"铁律（治现在场景穿帮、产品和背景分层）：**
- **禁止把产品当平面图层贴到 AI 背景上。** 场景图走 **image-to-image**，以真产品图为参考，让产品与环境**同一套打光**（方向/色温一致）、**有真实接触阴影与地面投影**、景深连贯。
- **自发光产品（灯具等）必须让自身的光溢到周围**（路面/植被/桌面被产品暖光照亮），否则"灯没在亮"一眼假。
- 目标：像"真的把产品摆进那个场景拍的"，不是合成贴图。

**技术随附：** 文件名含关键词；**alt 60-90 字符且每张不同**（描述该图具体内容与信息）；WebP/AVIF；首图 `fetchpriority="high"` 不懒加载。
