...
| `proof_scene`（证据场景） | gallery ≥2 张 + description 若干 | gallery + description | 用真实证据说服 | **每张对应一条已审卖点，把证据拍出来**：抗风=火焰迎风稳定；户外烹饪=真营地(帐篷/山景/黄昏)真点火锅里真在烧冒热气；便携=嵌套网袋挨着背包。**必须是真实使用场景，禁止干净摆拍充数**。**每张必须换一个场景母题（`scene_motif`），见下方「场景多样性」** |
...
- **`source_field` 白名单**：标准键（`lumens`、`color_temperature_k`、`battery_type`、`battery_capacity_mah`、`charge_time_h`、`runtime_h`、`ip_rating`、`dimensions.length|width|height`、`weight`、`material`、`mount_type`、`certifications`）**以及 `additional_specs.<key>`**（key=规格归一化后的稳定键，如 `additional_specs.capacity_pot`；该条目必须带 `label_en`，否则不可引用）。优先挑最能打消买家疑虑的 3-5 条。
...
- 产品的**主场景至少要出现一次**（露营产品就得有一张真营地：帐篷/篝火/山野/防潮垫），别让用途看不见。但**这是下限不是模板**——不许整套图都待在同一个环境里，见下方「场景多样性」。

**场景多样性铁律（2026-08-03 熊猫花洒教训，硬约束）：**

一个产品是被**不同的人、在不同的时刻**买走的。图组按"一条卖点一张图"排会踩坑：卖点往往全是规格（2.11 GPM 水流 / 6.5 ft 软管 / IPX8），而每条规格最省事的证明方式都是同一个场景 —— 于是 9 张图里 5 张是"营地里洗登山靴"，文案里写的宠物洗澡、孩子洗澡、花园浇水一张都没有。**一个熊猫造型的产品全套图在拍硬汉洗登山靴，造型和场景是割裂的。**

- **每张 `proof_scene` 必须声明 `scene_motif`**（snake_case，环境+人物，如 `backyard_dog_wash` / `toddler_bath_patio` / `campsite_gear_rinse`）；**任意两张 proof 不许同母题**（管线会校验，重复直接打回重排）。
- **整套 proof 至少覆盖 3 个不同母题**：环境要换，**用的人也要换**。
- **产品造型本身就是人群证据**：萌系/动物造型 = 买给孩子和宠物的，必须有对应母题；硬派技术造型 = 户外/工种。别让造型和场景打架。
- 排图组前先读 `operating_model.buyer_personas` 与 `target_customer_en` / `primary_use_case_en`，按**真实买家范围**铺开，而不是按卖点条数铺开。
- 几条卖点可以共用一个大环境，但每张图必须换**时刻/人物/具体环境**之一。拿不准时改场景，别改文案。

**工作原理硬约束（physics beats aesthetics）：**

`operating_model` 是从产品实拍图推导出来的"这东西到底怎么工作"，它**高于一切构图美感**：

- `hard_constraints` 里的每一条，在**任何展示产品工作状态的图里都必须成立**。潜水泵只有**整机浸没在水里**才出水；必须插电的设备不能画成无线运行的样子。
- `forbidden_depictions` 里的每一张画面**永远不许排进图组**（例："泵搁在干燥地面上却在喷水"）。
- 卖点与物理约束冲突时，**改场景直到成立**（把泵放进盆里/桶里/浴缸里），**绝不画不可能发生的状态**，也绝不因为"这样构图更好看"就把约束丢掉。
- 每张受约束影响的图，**把该约束原样写进它的 prompt**，别指望渲染模型自己记得。
