...
This runbook operates the Stage 3 text/emoji record runtime. Image/file assets
remain behind the reserved, fail-closed `ChatAssetStore` boundary and are not
placed in this database. Moments, voice, and video are separate product scopes.
...
| C19 Record PostgreSQL | Message bodies, order, idempotency/tombstones, receipts, event cursors, and mutation audit | Independent PostgreSQL 17 container, database, and named volume | PostgreSQL custom-format archive plus metadata |
| C19 assets | Reserved `ChatAssetStore` interface | No asset store, bytes, volume, or route | Stage 4 asset-VPS procedure |
...
`C19_RECORD_DATASET_ID` is the stable identity of one logical chat dataset. Keep
...
5. Review and merge the two exact C19 locations from
...
6. Verify with two authorized users: durable send, identical idempotent retry,
   history after a service restart, delivery/read advancement, unread state,
   SSE reconnect, and HTTP cursor recovery. A successful response must
   correspond to a record retrievable from the authoritative service.
...
private chat material, excluded from Git and ordinary Docker build contexts.
...
archive/metadata checksum, revision format, dataset identity, target environment,
...
3. verifies the Alembic revision and idempotency-ledger safety columns;
