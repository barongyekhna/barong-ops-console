...
`docs/C09_EXECUTION_PROVIDER_SEAL.md`。本文件定义的前端 Execution Provider shell
在 C09 范围内冻结为 read-only/no-execute 状态。C09G 没有修改 frontend runtime 逻辑，
没有新增 UI/API，没有新增 submit/run/execute/cancel/retry 入口，没有创建 execution
request，也没有连接 live provider。
...
C09G has sealed this frontend state:

- `ExecutionProviderStatusShell` remains status-only.
- frontend provider/action state remains `executable=false`.
- frontend provider/action state remains `can_request_execution=false`.
- the proxy remains exact GET-only for execution provider registry/access paths.
- no action submit payload, request id, idempotency key, task id, artifact ref, or operation log write
  is generated by the frontend.
- no live provider is connected.
- C09 is sealed as a capability layer, not an execution system launch.
