...
## 8. 机器人的注册规矩(2026-08-22 拍板,对所有数字员工生效)
**以后的机器人必须在控制台「用户管理 → 注册机器人」里登记**(owner 专用):登录名 / 显示名 / 岗位 / 所属组织 / 简介 / 登录密码。它建的是身份:`users` 行(viewer + `is_bot` 标记)+ C19 资料 + 组织成员关系,**零权限码**。
各员工模块自己的 bootstrap 脚本只负责 `agent_registry` 登记和重新收敛,不再是建号入口。
用户管理里的「删除」只删已停用、非 owner、无业务记录引用的账号(审批/评审/客服回信/自动化任务引用过的只能停用);C19 会话历史保留,成员关系/会话/权限/资料一并清掉。
