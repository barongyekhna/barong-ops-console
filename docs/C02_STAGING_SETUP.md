...
C02B 不启动 staging，不创建真实 `.env.staging`，不修改 production，不改
Nginx，不申请证书，也不接真实业务。
...
C02B 不运行这些命令。C02C 在确认 `.env.staging` 已经准备好之后，才可以使用
显式 project name 启动 staging：
...
真实存储或真实电商系统，就可能污染 production 数据。C02B/C02C 都只允许
foundation/demo 验证。
...
HTTPS、访问控制和 smoke check。C02B 不做这些事。
