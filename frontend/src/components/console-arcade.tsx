...
// 深空空战 —— 纯前端 canvas 纵版飞行射击（雷电 / 空战 风格）。
// WASD 控制方向，自动开火；躲子弹、打敌机、吃道具（加弹道 / 加攻速 / 回血）。
...
const POWER_LABEL: Record<Power["kind"], string> = {
  lane: "≡",
  rapid: "»",
  heal: "+",
};
const POWER_COLOR: Record<Power["kind"], string> = {
  lane: "#39d4ff",
  rapid: "#4dffa1",
  heal: "#ff6b8a",
};
...
export function ConsoleArcade() {
  ...
  const step = useCallback(
    (ts: number) => {
      ...
      lastRef.current = ts;
      const dt = dtMs / 16.6667; // 归一化到 ~60fps 步长
      g.elapsed += dtMs;
      ...
      if (g.spawnTimer <= 0) {
        ...
        g.enemies.push({
          ...
          vy: (1.1 + Math.random() * 0.9) * diff,
          hp: elite ? 3 : 1,
          r: elite ? 18 : 14,
          cd: 900 + Math.random() * 900,
          kind: elite ? 1 : 0,
        });
      }
      ...
      g.ebullets = g.ebullets.filter((b) => {
        b.x += b.vx * dt;
        b.y += b.vy * dt;
        return b.y < H + 12 && b.y > -12 && b.x > -12 && b.x < W + 12;
      });
      ...
      // 碰撞：敌机 / 敌机子弹 vs 玩家
      for (const e of g.enemies) {
        if (Math.hypot(g.px - e.x, g.py - e.y) < e.r + 12) {
          boom(g, e.x, e.y, "#ffb13b", 12);
          e.hp = 0;
          hitPlayer();
        }
      }
      ...
      for (const b of g.ebullets) {
        if (Math.hypot(g.px - b.x, g.py - b.y) < 12) {
          b.y = H + 999;
          hitPlayer();
        }
      }
      ...
      if (ctx) {
        ...
        for (const e of g.enemies) {
          ...
          ctx.fillStyle = e.kind === 1 ? "#ffb13b" : "#ff8a5f";
        ...
        for (const p of g.powers) {
          ...
          ctx.fill();
          ctx.globalAlpha = 1;
          ctx.strokeStyle = POWER_COLOR[p.kind];
          ctx.lineWidth = 1.6;
          ctx.beginPath();
          ...
          ctx.fillText(POWER_LABEL[p.kind], 0, 1);
          ctx.restore();
        }
        ...
        // HUD
        ctx.fillStyle = "#7d95ae";
        ctx.font = "700 12px ui-monospace, monospace";
        ctx.textAlign = "left";
        ctx.textBaseline = "top";
        ctx.fillText(`SCORE ${g.score}`, 14, 12);
        ctx.fillText(`弹道 x${g.lanes}`, 14, 30);
        ctx.textAlign = "right";
        ctx.fillStyle = "#39d4ff";
        let hearts = "";
        for (let i = 0; i < g.lives; i += 1) hearts += "▲ ";
        ctx.fillText(hearts.trim(), W - 14, 12);
      }

      // 每若干帧同步一次 HUD（供上方状态条显示）
      if (Math.floor(g.elapsed / 120) % 2 === 0) {
        setHud((prev) =>
          prev.score === g.score && prev.lives === g.lives && prev.lanes === g.lanes
            ? prev
            : { score: g.score, lives: g.lives, lanes: g.lanes },
        );
      }
  ...
  return (
    <section className="cc-arcade" aria-label="深空空战小游戏">
      <div className="cc-arcade-head">
        <div className="cc-arcade-title">
          ...
          <div>
            ...
            <strong>深空空战 · SKY RAID</strong>
          </div>
        </div>
        <div className="cc-arcade-meta">
          {phase === "playing" ? (
            <>
          ...
          <div className="cc-arcade-overlay">
            {phase === "over" ? (
              <>
                ...
                <span>本局得分 {hud.score} · 最高 {best}</span>
              </>
            ) : (
              <>
                <strong className="cc-arcade-big">深空空战</strong>
                <span>WASD 控制战机 · 自动开火 · 躲子弹 · 吃道具</span>
                <span className="cc-arcade-legend">
                  <em style={{ color: POWER_COLOR.lane }}>≡ 加弹道</em>
                  <em style={{ color: POWER_COLOR.rapid }}>» 加攻速</em>
                  <em style={{ color: POWER_COLOR.heal }}>+ 回血</em>
                </span>
              </>
            )}
            <button className="cc-arcade-btn" onClick={start} type="button">
              {phase === "over" ? (
