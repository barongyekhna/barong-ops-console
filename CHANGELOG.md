...
> 这份文件是**给读仓库的人看的**：按提交粒度、技术口径记录。
> 控制台里那个「版本更新」面板是**给使用控制台的人看的**：按版本粒度、
> 人话口径。两者服务不同读者，内容不必一一对应——但版本号必须一致。

## [Unreleased]

### Added
- 多组织员工:用户管理页可改岗位与主组织,并按公司勾选成员关系(多组织的人登录后顶栏切换公司);组织上下文对「多成员关系但会话未选组织」的人落在主组织,不再整站 403。用户接口回 `memberships`。
- M 系列库存门禁放开(用户 2026-09-10 拍板):除 owner / 制造公司超管外,制造公司成员在权限页拿到 `mfg.inventory.read` 可查、`mfg.inventory.manage` 可写;接口分读写两道依赖,主页库存卡与霓旌(查/写)走同一道门,只读账号开卡会被霓旌明确回绝。
- M 系列库存:物料/成品编码自动生成。成品 = 系列码-三位流水(`TBL-001`),物料 = 大类码-四位流水(`PK-0001`),各组各自发号、锁行取号、跳过手填占用的号;新表 `mfg_code_groups`,物料大类预置十个,成品系列随第一个成品建立,组码按名字给建议(中文取拼音首字母)。手填编码保留为逃生口;没有流水的主档可改编码。迁移 `20260910_01_mfg_code_groups`;新依赖 `pypinyin`。

### Fixed
- VPN：登记接口拒绝 0.3.0 以下的客户端（HTTP 426），页面按本机上报版本在点连接之前就提示「太旧」并给出下载按钮；此前 0.2.1 客户端登记返回 201 却永远握不上手（隧道配置烧死在旧端口），还会重新启用停用的老设备。

## [2.1.0] - 2026-09-03

把 2.0.0 修通的功能底下的重复与隐患处理掉：同一段请求代码在 22 个文件各抄一份、
同一套串行派单队列在 5 个模块各写一遍、同一张样式表堆了三代互相覆盖的旧皮肤、
K 的接口层 6763 行里塞着 94 个私有 helper。

### Fixed

- 19 个「当场等 AI / 等外部服务返回」的接口此前没有任何超时；收口到统一入口后
  会吃到 15 秒默认值，逐个按后端实际耗时显式放宽（依据是后端 docstring、
  服务里的出网调用、或后端自己的超时常量，不靠猜）。
- `w/siteops` 的运单刷新：后端 `TRACK17_TIMEOUT_SECONDS = 15` 与前端默认超时**完全相等**，
  承运商稍慢就在界面上变成失败。改为 45 秒，让后端先超时并给出人话。
- 并发上传被 in-flight 去重误合并：`FormData` 走 `Object.keys()` 是空的，
  两个内容不同的上传算出同一个缓存 key，第二个拿到第一个的响应。
- `apiRequest` 遇 204/205 直接 `response.json()` 抛 `SyntaxError`；DELETE 类接口大量返回 204。
- `apiRequest` 把 `FormData` 当 JSON 序列化并手动设 `Content-Type`，破坏 multipart boundary。
- `ApiError` 丢弃后端的结构化 `detail`（K 的发布门禁 blockers、M 的缺料清单静默消失）。
- b2b 三个模块的错误文案在收口中途退化成通用兜底，丢掉状态码；已补回 `${label}（${status}）`。
- n8n 派单的回调地址根为空时静默发出相对地址；改为当场抛错。
  （`PUBLIC_API_BASE_URL` 在生产容器里从未定义，一直靠 `P_CALLBACK_BASE` 掩盖。）
- 观测数据无留存上限。

### Changed

- 前端 22 个模块、197 处裸 fetch 收口到 `lib/api.ts`；新增 `lib/labelled-api.ts`
  承载两种带模块标签的错误口径（此前各被复制 8 份）。
- 5 份 n8n 串行队列副本合并为 `services/n8n_dispatch.py`。
  **刻意不合**：`w_series/logistics.py`（按 target 串行 + definitive/uncertain 错误分类）、
  `h_series/sitehealth`（根本没有队列）、以及 `record_result`
  （P 有「迟到的 success 可翻案看门狗误判」的例外）。
- `globals.css` 8631 → 7671 行：删掉 169 个整块死规则；三代基座补上层标记；
  层目录 11 → 21 条。**未做**「合并四份外壳定义」——违反文件顶部「绝不回头改旧层」的家规。
- `k_series/product_knowledge/router.py` 6763 → 4251 行，2851 行搬进 `services/` 六个模块。
  router 尾部保留 re-export 兼容层，22 个存量测试文件无需改动。

### Removed

- 全仓 `Authorization: Bearer` 死代码归零（19 处 getItem、0 处 setItem，
  代理 `route.ts` 也不读该头；认证一直靠同源 Cookie）。
- 停用体检期临时账号及其名下的接入凭据。

### Security

- `.gitignore` 的 `.env.production` 是精确文件名，不覆盖 `.env.production.bak-*`；
  两个含 13 / 9 行真实密钥的备份处在「未跟踪但也未忽略」状态，一次 `git add -A` 即入库。
  已补规则（含正反对照验证）并把备份移出工作树。
- 三处含外部平台凭据的配置文件从 `0644` 收紧到 `0600`。

### Tests

- 队列不变量、上架门禁的安全形状校验、接口客户端的超时/并发/204/FormData 行为
  从「读源码匹配字符串」改为真实调用验证。
- 新增 `tests/frontend/_alias-hooks.mjs`：让前端模块第一次能被 `node --test` 加载
  （此前 `@/` 别名 + TS 参数属性两道障碍，导致关于 `apiRequest` 的断言只能读源码）。
- 新增 K 路由清单基准：81 条逐条快照、5 组双路径别名、双前缀挂载一致、47 个兼容符号。
  这套守护在本轮重构中三次挡住会让路由静默消失的错误。

## [2.0.0] - 2026-09-02

第一个正式切出的版本。此前所有变更都堆在 `[Unreleased]` 下，从未发版收口；
这一版把它们全部收进来，并加上 2026-08-31 全平台体检后的九阶段全量修复。

版本号同时从 `C-SERIES-V1.1.0` 改为 `2.0.0`——「C-SERIES」是当年只有
RBAC/模块/API 密钥时的子系统代号，早已不描述现在这个装了十几个业务系列的产品。
...
- 用户管理新增「注册机器人」(owner 专用:viewer + 机器人标记 + 所属组织 + C19
  资料,零权限码;以后的数字员工必须在这里登记)和「彻底删除」(只删已停用、非
  owner、无业务记录引用的账号,清掉会话/成员关系/权限/通讯资料);清掉 27 个
  C19 验收遗留的临时账号。
- 数字员工 02「霓旌」(制造公司库管员)上线:在 C19 通讯里听懂入库/生产/
  发货/盘点调整,进程内以说话人的身份过 M 系列角色门开单(她自己零权限),
  每一笔写入先出确认卡;模型只做意图抽取,数量必须在原话里出现;一句话多
  件事拒绝;新建物料/改 BOM 红灯;撤销=反向盘点调整。新增 `agent_series/common/`
  通用 C19 客户端与记忆存储,容器 `nijing-worker`,台账桶 `agent_chat_nijing_deepseek`。
